...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        all_CPT = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variable in eliminationOrder:
            all_CPT, joined_factor = joinFactorsByVariable(
                all_CPT, variable)

            if len(joined_factor.unconditionedVariables()) > 1:
                marginal_joint = eliminate(
                    joined_factor, variable)
                all_CPT.append(marginal_joint)

        full_joint = joinFactors(all_CPT)
