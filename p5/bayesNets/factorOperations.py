...
def joinFactors(factors):
    ...
    for factor in factors:
        ...
        unconditioned = unconditioned.union(
            factor.unconditionedVariables())

    for u in unconditioned:
        if u in conditioned:
            conditioned.remove(u)

    joinedFactor = Factor(unconditioned, conditioned,
    ...
    for assignment in assignments:
        ...
        for f in factors:
            joinedFactor.setProbability(assignment, joinedFactor.getProbability(
                assignment) * f.getProbability(assignment))
    return joinedFactor
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        newFactor = Factor(
            unconditioned, factor.conditionedVariables(), factor.variableDomainsDict())
...
def normalize(factor):
    ...
    union = unconditionedVar.union(conditionedVar)

    unconditionedVarCp = unconditionedVar.copy()

    for unconditioned in unconditionedVar:
        var = factor.variableDomainsDict()[unconditioned]
        if len(var) == 1:
            unconditionedVarCp.remove(unconditioned)
            conditionedVar.add(unconditioned)

    assignments = factor.getAllPossibleAssignmentDicts()

    f = Factor(unconditionedVarCp, conditionedVar, variableDomainsDict)

    for assignment in assignments:
        f.setProbability(assignment, factor.getProbability(
            assignment) / totalProb)
