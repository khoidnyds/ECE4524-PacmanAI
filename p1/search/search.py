...
def depthFirstSearch(problem):
    ...
    visited = set()
    fringe.push((problem.getStartState(), []))  # push start node to stack

    while not fringe.isEmpty():
        curNode, curMove = fringe.pop()
        if curNode not in visited:
            ...
            if problem.isGoalState(curNode):
                ...
                for nextNode, nextMove, _ in problem.getSuccessors(curNode):
                    fringe.push((nextNode, curMove + [nextMove]))
    return []
...
def uniformCostSearch(problem):
    ...
    # NOTE: cost == priority
    cost = 0
    fringe.push((problem.getStartState(), [], cost), 0)

    while not fringe.isEmpty():
        curNode, curMove, curCost = fringe.pop()
        if curNode not in visited or curCost < visited[curNode]:
            ...
            if problem.isGoalState(curNode):
                ...
                for nextNode, nextMove, nextCost in problem.getSuccessors(curNode):
                    fringe.push((nextNode, curMove + [nextMove], curCost + nextCost), curCost + nextCost)
    return []
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not fringe.isEmpty():
        ...
        if problem.isGoalState(curNode):
            ...
            for nextNode, nextMove, _ in problem.getSuccessors(curNode):
                newCost = problem.getCostOfActions(curMove + [nextMove])
                newNode = (nextNode, curMove + [nextMove], newCost)

                update = True
                for node in visited:
                    visitedNode, visitedCost = node
                    if nextNode == visitedNode and newCost >= visitedCost:
                        update = False

                if update:
                    fringe.push(newNode, newCost + heuristic(nextNode, problem))
                    visited.append((nextNode, newCost))
