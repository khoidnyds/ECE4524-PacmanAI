...
class CornersProblem(search.SearchProblem):
  ...
  y |
    ...
    # OOP design. State class ({corners:visited status}, current position)
    class State():
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # all attributes are initialized in State object
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.State(self.corners, self.startingPosition)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                corner_state_copy.visited(point)
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    corners = state.get_corner()
    unvisited_corners = []
    for key, value in corners.items():
        if not value:
            unvisited_corners.append(key)
...
def getMaxDistance(food, x, y):
    ...
    for f in food:
        # minDistance = min(minDistance, util.manhattanDistance(f, posTuple))
        if (util.manhattanDistance(f, posTuple) > maxDistance):
            maxFood = f
            maxDistance = util.manhattanDistance(f, posTuple)

    # maxDistance = max(maxDistance, util.manhattanDistance(f, posTuple))
    # distanceList.append(util.manhattanDistance(f, posTuple))
    # distSum = distSum + util.manhattanDistance(f, posTuple)
...
def foodHeuristic(state, problem):
    ...
    if len(foodMap) == 0:
        return 0
    # Using mazeDistance to take account for the walls
    distance = 0
    for food in foodMap:
